<template>
  <div class="bg-gray-100 border-b-2 border-gray-700">
    <nav class="relative px-2 flex justify-between items-center">
      <nuxt-link class="text-3xl font-bold flex items-center" to="/">
        <img class="h-16" src="../assets/img/pencil_logo.png" />
        <div class="text-3xl">DraWin</div>
      </nuxt-link>
      <div class="lg:hidden">
      ...
      >
        <li>
          <nuxt-link class="text-lg text-gray-600 font-semibold" to="/gallery"
            >Gallery</nuxt-link
          >
        </li>
      ...
      <div v-if="!user.online">
        <nuxt-link
          class="hidden lg:inline-block lg:ml-auto lg:mr-3 py-2 px-6 bg-white text-lg text-gray-900 font-bold border-2 rounded-xl transition duration-200 hover:border-yellow-400"
          to="/login"
          >Sign In</nuxt-link
        >
        <nuxt-link
          class="hidden lg:inline-block py-2 px-6 bg-blue-500 hover:bg-blue-600 text-lg text-white font-bold rounded-xl transition duration-200"
          to="/register"
          >Sign up</nuxt-link
        >
      </div>
      <div v-else
        class="flex text-xl justify-center px-4 items-center"
      >
        <span class="pr-3 text-gray-600">{{ user.name }}</span>
        <a class="border-transparent text-red-900 hover:border-l-4 border-red-500 group border-l-4 px-3 py-2 flex items-center text-sm " @click="DeleteUser()">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="text-red-600 group-hover:text-red-600 flex-shrink-0 -ml-1 mr-3 h-6 w-6"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
          ...
          <button class="text-red-600 font-bold truncate">
            Disconnect
          </button>
        </a>
      </div>
